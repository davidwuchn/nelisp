...
if [ "$(uname -s)" = "Darwin" ]; then
  ...
  if [ "$INCLUDE_TARBALL" -eq 1 ]; then
    ...
    tools/build-standalone-tarball.sh v1.0.0 macos-aarch64 --emacs "$EMACS"
    if [ "$BUILD_ONLY_STANDALONE_SMOKES" -eq 1 ]; then
      tools/verify-standalone-tarball.sh v1.0.0 macos-aarch64 --layout-only
    else
      tools/verify-standalone-tarball.sh v1.0.0 macos-aarch64
    fi
...
if [ "$INCLUDE_TARBALL" -eq 1 ]; then
  ...
  tools/build-standalone-tarball.sh v1.0.0 linux-x86_64 --emacs "$EMACS"
  tools/verify-standalone-tarball.sh v1.0.0 linux-x86_64
